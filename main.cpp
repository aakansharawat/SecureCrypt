...
// Assuming these are defined elsewhere and necessary for compilation
...
/**
 * @brief Processes a single encryption or decryption task.
 * * Takes a string in the format "ACTION,FILEPATH[,KEY]" and performs 
 * the corresponding file operation using a derived key.
 * * @param taskData The task string.
...
int processCryptionTask(const string& taskData) {
    ...
    // FNV-1a 32-bit Hash Function (for password key derivation)
    ...
    // Derived Key (1-byte int key for Caesar cipher)
    ...
    if (action == "encrypt") {
        ...
        if (!secretKey.empty()) {
            // Caller provided a password: verify and derive key
            if (hasLock) {
    ...
    // --- 4. File Modification (The Fix is Here) ---
    {
        IO io(filePath); 
        ...
        char ch;
        // Loop through file contents byte by byte
        while (f_stream.get(ch)) {
            
            // 1. Calculate the position of the byte we just read (current read pointer - 1)
            streampos pos_to_write = f_stream.tellg();
            pos_to_write -= 1; 
            
            unsigned char uch = static_cast<unsigned char>(ch);

            // 2. Perform the byte modification (Caesar cipher variant)
            if (action == "encrypt") {
                uch = static_cast<unsigned char>((uch + key) % 256);
            } else { 
                // Note: Adding 256 ensures positive result before final modulo
                uch = static_cast<unsigned char>((uch - key + 256) % 256);
            }

            // 3. Move the write pointer and write the modified byte
            f_stream.seekp(pos_to_write);
            f_stream.put(static_cast<char>(uch));
            
            // 4. CRITICAL FIX: Reset stream state after writing for the next read
            // Writing switches the stream to output mode, which prevents the next f_stream.get(ch) from working.
            f_stream.clear(); // Clear EOF/Fail flags that might have been set by put/seekp

            // 5. Reposition the read pointer to the next byte
            f_stream.seekg(pos_to_write + 1);
        }
    } // IO object goes out of scope here, triggering file close/flush

    // --- 5. Post-processing: Rename and Sidecar Policy ---
    if (action == "encrypt") {
        if (hasPassword) {
